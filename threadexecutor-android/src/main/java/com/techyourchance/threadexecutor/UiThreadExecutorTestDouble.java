...
/**
 * Test double of {@link UiThreadExecutor} that can be used in tests in order to establish
 ...
 */
public class UiThreadExecutorTestDouble extends UiThreadExecutor {
        ...
        synchronized (MONITOR) {
            Thread worker = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    UiThreadExecutorTestDouble.this.join();
                    runnable.run();
                }
            });
            mThreads.add(worker);
    ...
    public void join() {
        ...
        while ((thread = threadsCopy.poll()) != null) {
            try {
                ...
                // due to the way execute(Runnable) is being implemented, this method will be called
